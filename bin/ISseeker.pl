...
#		}
	...
	for my $flank (sort { $b->{is_location} cmp $a->{is_location} } @good_flanks)
	{
		...
		{
			##
			## Check that flank has not already been output
			##
			if (!$flank->{SQL_output})
			{
			        $flank->{SQL_output} = 1;
				...
				{
					##
					## Add mate flank right after its mate
					##
				        $flank->{mate}->{SQL_output} = 1;
					$log->debug("Adding linked mate IS...\n");
					## Add SQL for mate using LAST_INSERT_ID for link
